...
async function runClusterScan(){
    ...
    btn.querySelector('.btn-loader').classList.remove('hidden');btn.disabled=true;
    document.getElementById('cluster-results').classList.add('hidden');
    const body={
        min_insiders:parseInt(document.getElementById('cluster-min').value),
        days:parseInt(document.getElementById('cluster-days').value),
        min_total_value:parseInt(document.getElementById('cluster-value').value),
        senior_required:document.getElementById('cluster-senior').value==='true'
    };
    try{
        const data=await(await fetch('/api/insider_clusters',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})).json();
        btn.querySelector('.btn-text').textContent='find clusters';btn.querySelector('.btn-loader').classList.add('hidden');btn.disabled=false;
...
function renderClusters(data){
    ...
    document.getElementById('cluster-title').textContent=`${data.total_found} cluster${data.total_found===1?'':'s'} found`;
    const c=document.getElementById('cluster-table-container');
    if(!data.clusters||!data.clusters.length){c.innerHTML='<p class="no-trades">no clusters match these filters - try loosening the min insiders or value thresholds</p>';return}
    let h=`<table class="screener-table"><thead><tr><th>#</th><th>ticker</th><th>name</th><th>sector</th><th>insiders</th><th>trades</th><th>senior</th><th>total $</th><th>most recent</th><th>price</th><th>move since</th><th>confidence</th></tr></thead><tbody>`;
    data.clusters.forEach((r,i)=>{
        const cc=r.confidence>=70?'var(--green)':r.confidence>=50?'var(--amber)':'var(--red)';
        const mc=(r.move_since_cluster||0)>=0?'var(--green)':'var(--red)';
        const moveStr=r.move_since_cluster!=null?`${r.move_since_cluster>=0?'+':''}${r.move_since_cluster}%`:'-';
        h+=`<tr><td>${i+1}</td>
            <td><span class="ticker-link" onclick="loadFromScreener('${r.ticker}')">${r.ticker}</span></td>
            <td style="font-size:11px">${r.name||'-'}</td>
            <td style="font-size:10px;color:var(--text-muted)">${r.sector||'?'}</td>
            <td style="font-weight:700">${r.n_insiders}</td>
            <td>${r.n_trades}</td>
            <td style="color:var(--accent)">${r.senior_count}</td>
            <td>${fmtVal(r.total_value)}</td>
            <td style="font-size:10px">${r.most_recent_date}</td>
            <td>${r.current_price?'$'+r.current_price.toFixed(2):'-'}</td>
            <td style="color:${mc};font-weight:600">${moveStr}</td>
            <td style="color:${cc};font-weight:700">${r.confidence}</td>
        </tr>`;
    });
    h+='</tbody></table>';c.innerHTML=h;
}
...
async function runPeadScan(){
    const btn=document.getElementById('pead-btn');
    const count=parseInt(document.getElementById('pead-count').value);
    btn.querySelector('.btn-text').textContent='scanning...';
    btn.querySelector('.btn-loader').classList.remove('hidden');btn.disabled=true;
    document.getElementById('pead-progress').classList.remove('hidden');
    document.getElementById('pead-results').classList.add('hidden');
    try{
        await fetch('/api/pead_scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({count})});
        const poll=setInterval(async()=>{
            try{
                const s=await(await fetch('/api/pead_status')).json();
                document.getElementById('pead-progress-fill').style.width=s.progress+'%';
                document.getElementById('pead-progress-text').textContent=s.progress+'%';
                document.getElementById('pead-progress-message').textContent=s.message;
                if(s.complete){
                    clearInterval(poll);
                    btn.querySelector('.btn-text').textContent='scan for drift';
                    btn.querySelector('.btn-loader').classList.add('hidden');btn.disabled=false;
                    renderPead(s.results,s.total_scanned||count);
                }
                if(s.error){
                    clearInterval(poll);
                    btn.querySelector('.btn-text').textContent='scan for drift';
                    btn.querySelector('.btn-loader').classList.add('hidden');btn.disabled=false;
                    showStatus('error: '+s.error,'error');
                }
            }catch(e){}
        },1500);
    }catch(e){
...
function renderPead(results,total){
    document.getElementById('pead-results').classList.remove('hidden');
    document.getElementById('pead-title').textContent=`${results.length} drift signal${results.length===1?'':'s'} (${total} scanned)`;
    const c=document.getElementById('pead-table-container');
    if(!results.length){c.innerHTML='<p class="no-trades">no active PEAD signals - either no recent earnings beats/misses or the drift window has passed</p>';return}
    let h=`<table class="screener-table"><thead><tr><th>#</th><th>ticker</th><th>name</th><th>sector</th><th>earnings date</th><th>days since</th><th>surprise</th><th>direction</th><th>EPS est / actual</th><th>price</th><th>move since</th><th>confidence</th></tr></thead><tbody>`;
    results.forEach((r,i)=>{
        const cc=r.confidence>=60?'var(--green)':r.confidence>=40?'var(--amber)':'var(--red)';
        const dc=r.direction==='up'?'var(--green)':'var(--red)';
        const sc=r.surprise_pct>=0?'var(--green)':'var(--red)';
        const mc=r.move_since_earnings>=0?'var(--green)':'var(--red)';
        h+=`<tr><td>${i+1}</td>
            <td><span class="ticker-link" onclick="loadFromScreener('${r.ticker}')">${r.ticker}</span></td>
            <td style="font-size:11px">${r.name||'-'}</td>
            <td style="font-size:10px;color:var(--text-muted)">${r.sector||'?'}</td>
            <td style="font-size:10px">${r.earnings_date}</td>
            <td>${r.days_since_earnings}d</td>
            <td style="color:${sc};font-weight:700">${r.surprise_pct>=0?'+':''}${r.surprise_pct}%</td>
            <td><span class="action-tag ${r.direction==='up'?'buy':'sell'}" style="text-transform:uppercase">${r.direction}</span></td>
            <td style="font-size:10px">${r.eps_estimate} / ${r.eps_actual}</td>
            <td>$${r.current_price}</td>
            <td style="color:${mc};font-weight:600">${r.move_since_earnings>=0?'+':''}${r.move_since_earnings}%</td>
            <td style="color:${cc};font-weight:700">${r.confidence}</td>
        </tr>`;
    });
    h+='</tbody></table>';c.innerHTML=h;
...
/* ----- MEAN REVERSION ----- */
async function refreshVixStatus(){
    try{
        const v=await(await fetch('/api/vix_now')).json();
        const bar=document.getElementById('vix-status');
        const gateOpen=v.gate_open_default;
        bar.className='vix-status-bar '+(gateOpen?'vix-calm':'vix-turbulent');
        bar.innerHTML=`<span class="vix-label">VIX</span><span class="vix-value">${v.vix}</span><span class="vix-state">${gateOpen?'CALM - gate likely open':'TURBULENT - gate likely closed'}</span>`;
    }catch(e){}
}

async function runMrScan(){
    const btn=document.getElementById('mr-btn');
    const count=parseInt(document.getElementById('mr-count').value);
    const max_vix=parseInt(document.getElementById('mr-vix').value);
    btn.querySelector('.btn-text').textContent='scanning...';
    btn.querySelector('.btn-loader').classList.remove('hidden');btn.disabled=true;
    document.getElementById('mr-progress').classList.remove('hidden');
    document.getElementById('mr-results').classList.add('hidden');
    //stall detector: if progress doesn't move for this long, assume the
    //backend died (eg flask reloader kicked in) and stop polling
    const STALL_MS=90000;
    let lastProgress=-1, lastChangeAt=Date.now();
    const resetBtn=()=>{
        btn.querySelector('.btn-text').textContent='scan setups';
        btn.querySelector('.btn-loader').classList.add('hidden');
        btn.disabled=false;
    };
    try{
        await fetch('/api/mean_reversion_scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({count,max_vix})});
        const poll=setInterval(async()=>{
            try{
                const s=await(await fetch('/api/mr_status')).json();
                document.getElementById('mr-progress-fill').style.width=s.progress+'%';
                document.getElementById('mr-progress-text').textContent=s.progress+'%';
                document.getElementById('mr-progress-message').textContent=s.message;
                if(s.progress!==lastProgress){lastProgress=s.progress;lastChangeAt=Date.now();}
                if(s.complete){
                    clearInterval(poll);resetBtn();renderMr(s.results);return;
                }
                if(s.error){
                    clearInterval(poll);resetBtn();
                    showStatus('error: '+s.error,'error');return;
                }
                //stall guard
                if(Date.now()-lastChangeAt>STALL_MS){
                    clearInterval(poll);resetBtn();
                    showStatus('scan stopped responding. backend may have restarted. check the console and try again.','error');
                }
            }catch(e){
                if(Date.now()-lastChangeAt>STALL_MS){
                    clearInterval(poll);resetBtn();
                    showStatus('lost contact with scanner. check the console and try again.','error');
                }
            }
        },1500);
    }catch(e){resetBtn();}
...
function renderMr(result){
    document.getElementById('mr-results').classList.remove('hidden');
    const bar=document.getElementById('vix-status');
    bar.className='vix-status-bar '+(result.gate_open?'vix-calm':'vix-turbulent');
    bar.innerHTML=`<span class="vix-label">VIX</span><span class="vix-value">${result.vix_level}</span><span class="vix-state">${result.message}</span>`;
    document.getElementById('mr-title').textContent=`${result.signals.length} setup${result.signals.length===1?'':'s'} found`;
    const c=document.getElementById('mr-table-container');
    if(!result.gate_open){c.innerHTML=`<p class="no-trades">${result.message}. mean reversion is unreliable when markets are turbulent - wait for VIX to settle or raise the threshold.</p>`;return}
    const minConf=parseInt(document.getElementById('mr-min-conf').value||'0');
    const filtered=result.signals.filter(s=>s.confidence>=minConf);
    document.getElementById('mr-title').textContent=`${filtered.length} of ${result.signals.length} setup${result.signals.length===1?'':'s'} (min confidence ${minConf})`;
    if(!filtered.length){
        if(!result.signals.length)c.innerHTML='<p class="no-trades">no stocks meeting all four gates right now - try widening the scan count</p>';
        else c.innerHTML=`<p class="no-trades">${result.signals.length} setups found but none above confidence ${minConf}. lower the filter to see what is available.</p>`;
        return;
    }
    let h=`<table class="screener-table"><thead><tr><th>#</th><th>ticker</th><th>name</th><th>sector</th><th>direction</th><th>RSI</th><th>BB pos</th><th>vs 200dma</th><th>vol ratio</th><th>price</th><th>confidence</th></tr></thead><tbody>`;
    filtered.forEach((r,i)=>{
        const cc=r.confidence>=55?'var(--green)':r.confidence>=35?'var(--amber)':'var(--red)';
        const rc=r.rsi<30?'var(--green)':r.rsi>70?'var(--red)':'var(--amber)';
        const tc=r.price_vs_sma200_pct>=0?'var(--green)':'var(--red)';
        h+=`<tr><td>${i+1}</td>
            <td><span class="ticker-link" onclick="loadFromScreener('${r.ticker}')">${r.ticker}</span></td>
            <td style="font-size:11px">${r.name||'-'}</td>
            <td style="font-size:10px;color:var(--text-muted)">${r.sector||'?'}</td>
            <td><span class="action-tag ${r.direction}">${r.direction.toUpperCase()}</span></td>
            <td style="color:${rc};font-weight:700">${r.rsi}</td>
            <td>${r.bb_position}%</td>
            <td style="color:${tc};font-weight:600">${r.price_vs_sma200_pct>=0?'+':''}${r.price_vs_sma200_pct}%</td>
            <td>${r.vol_ratio}x</td>
            <td>$${r.current_price}</td>
            <td style="color:${cc};font-weight:700">${r.confidence}</td>
        </tr>`;
...
//refresh VIX status when the page first loads
refreshVixStatus();
...
const STRATEGY_LABELS = {
    ensemble:'ensemble', cluster:'insider cluster', insider_cluster:'insider cluster', pead:'earnings drift',
    meanrev:'mean reversion', mean_reversion:'mean reversion', momentum:'relative momentum', momentum_12_1:'12-1 momentum',
    week52_high:'52w high', quality:'quality', shareholder_yield:'shareholder yield', low_beta_trend:'low-beta trend',
    ai_paper_cluster:'AI cluster', ai_paper_pead:'AI earnings drift', ai_paper_meanrev:'AI mean reversion',
    ai_paper_momentum:'AI relative momentum', ai_paper_momentum_12_1:'AI 12-1 momentum', ai_paper_week52_high:'AI 52w high',
    ai_paper_quality:'AI quality', ai_paper_shareholder_yield:'AI shareholder yield', ai_paper_low_beta_trend:'AI low-beta trend',
    screener:'screener', manual:'manual'
};
const STRATEGY_COLOURS = {
    ensemble:'#4a7cff', cluster:'#2dd4a0', insider_cluster:'#2dd4a0', pead:'#f0a030',
    meanrev:'#a07cff', mean_reversion:'#a07cff', momentum:'#45c0ff', momentum_12_1:'#40c8e0', week52_high:'#2dd4a0',
    quality:'#8bd450', shareholder_yield:'#f5c542', low_beta_trend:'#80d455',
    ai_paper_cluster:'#2dd4a0', ai_paper_pead:'#f0a030', ai_paper_meanrev:'#a07cff',
    ai_paper_momentum:'#45c0ff', ai_paper_momentum_12_1:'#40c8e0', ai_paper_week52_high:'#2dd4a0',
    ai_paper_quality:'#8bd450', ai_paper_shareholder_yield:'#f5c542', ai_paper_low_beta_trend:'#80d455',
    screener:'#40c8e0', manual:'#8888a0'
};
let logFilter = 'all';
let logCache = null;
...
document.addEventListener('DOMContentLoaded', () => {
    const dt = document.getElementById('log-entry-date');
    if (dt) dt.value = new Date().toISOString().split('T')[0];
});

async function logTrade(){
    const body = {
        ticker: document.getElementById('log-ticker').value.trim().toUpperCase(),
        strategy: document.getElementById('log-strategy').value,
        action: document.getElementById('log-action').value,
        entry_price: parseFloat(document.getElementById('log-entry-price').value) || 0,
        shares: parseFloat(document.getElementById('log-shares').value) || 0,
        currency: document.getElementById('log-currency').value,
        entry_date: document.getElementById('log-entry-date').value || new Date().toISOString().split('T')[0],
        target_price: parseFloat(document.getElementById('log-target').value) || null,
        stop_price: parseFloat(document.getElementById('log-stop').value) || null,
        notes: document.getElementById('log-notes').value
    };
    if (!body.ticker || !body.entry_price){ showStatus('ticker and entry price required','error'); return; }
    try {
        const r = await fetch('/api/log_trade', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
        const d = await r.json();
        if (d.error){ showStatus('error: '+d.error,'error'); return; }
        showStatus('trade logged','success');
        ['log-ticker','log-entry-price','log-shares','log-target','log-stop','log-notes'].forEach(id => document.getElementById(id).value = '');
        refreshTradeLog();
    } catch(e) { showStatus('error: '+e.message,'error'); }
}
...
async function refreshTradeLog(){
    try {
        const d = await(await fetch('/api/trade_log')).json();
        if (d.error){ showStatus('error: '+d.error,'error'); return; }
        logCache = d;
        renderTradeLogSummary(d.summary);
        renderTradeLogList(d.trades);
    } catch(e) {}
}

function renderTradeLogSummary(s){
    const sec = document.getElementById('log-summary-section');
    if (!s || !s.total_trades){ sec.classList.add('hidden'); document.getElementById('log-strategy-section').classList.add('hidden'); return; }
    sec.classList.remove('hidden');
    const wrc = s.win_rate >= 55 ? 'var(--green)' : s.win_rate >= 45 ? 'var(--amber)' : 'var(--red)';
    const avgc = s.avg_realised_pct >= 0 ? 'var(--green)' : 'var(--red)';
    document.getElementById('log-summary-grid').innerHTML = `
        <div class="info-card primary"><span class="info-label">total trades</span><span class="info-value big">${s.total_trades}</span></div>
        <div class="info-card"><span class="info-label">open</span><span class="info-value">${s.open}</span></div>
        <div class="info-card"><span class="info-label">closed</span><span class="info-value">${s.closed}</span></div>
        <div class="info-card"><span class="info-label">win rate</span><span class="info-value" style="color:${wrc}">${s.win_rate}%</span></div>
        <div class="info-card"><span class="info-label">avg return per trade</span><span class="info-value" style="color:${avgc}">${s.avg_realised_pct>=0?'+':''}${s.avg_realised_pct}%</span></div>
        <div class="info-card"><span class="info-label">total realised</span><span class="info-value" style="color:${avgc}">${s.total_realised_pct>=0?'+':''}${s.total_realised_pct}%</span></div>
    `;

    const stratSec = document.getElementById('log-strategy-section');
    if (Object.keys(s.by_strategy||{}).length){
        stratSec.classList.remove('hidden');
        document.getElementById('log-strategy-grid').innerHTML = Object.entries(s.by_strategy).map(([k,v])=>{
            const label = STRATEGY_LABELS[k] || k;
            const colour = STRATEGY_COLOURS[k] || '#8888a0';
            const wrc = v.win_rate >= 55 ? 'var(--green)' : v.win_rate >= 45 ? 'var(--amber)' : 'var(--red)';
            const avgc = (v.avg_pct||0) >= 0 ? 'var(--green)' : 'var(--red)';
            return `<div class="baseline-card" style="border-color:${colour}33">
                <div class="baseline-name" style="color:${colour}">${label}</div>
                <div class="baseline-acc" style="color:${wrc}">${v.win_rate}%</div>
                <div class="baseline-note">${v.closed} closed${v.open?` · ${v.open} open`:''} · avg <span style="color:${avgc}">${v.avg_pct>=0?'+':''}${v.avg_pct}%</span></div>
            </div>`;
        }).join('');
    } else stratSec.classList.add('hidden');
}

function renderTradeLogList(trades){
    const sec = document.getElementById('log-list-section');
    if (!trades || !trades.length){ sec.classList.add('hidden'); return; }
    sec.classList.remove('hidden');
    const filtered = trades.filter(t => logFilter === 'all' || t.status === logFilter)
                           .sort((a,b) => (b.id||0) - (a.id||0));
    document.getElementById('log-list-title').textContent = `${filtered.length} ${logFilter === 'all' ? 'total' : logFilter} trade${filtered.length===1?'':'s'}`;
    const c = document.getElementById('log-list-container');
    if (!filtered.length){ c.innerHTML = '<p class="no-trades">no trades match this filter</p>'; return; }

    let h = `<table class="screener-table"><thead><tr><th>ticker</th><th>strategy</th><th>action</th><th>entry</th><th>shares</th><th>now</th><th>days</th><th>P/L %</th><th>P/L value</th><th>status</th><th>notes</th><th></th></tr></thead><tbody>`;
    filtered.forEach(t => {
        const pl = t.status === 'closed' ? t.realised_pl_pct : (t.unrealised_pl_pct_gbp != null ? t.unrealised_pl_pct_gbp : t.unrealised_pl_pct);
        const plc = pl == null ? 'var(--text-muted)' : pl >= 0 ? 'var(--green)' : 'var(--red)';
        const plv = t.status === 'closed'
            ? (t.realised_pl_pct != null ? (t.realised_pl_pct/100 * t.entry_price * t.shares) : null)
            : (t.unrealised_pl_gbp != null ? t.unrealised_pl_gbp : t.unrealised_pl_value);
        const stratLabel = STRATEGY_LABELS[t.strategy] || t.strategy;
        const stratColour = STRATEGY_COLOURS[t.strategy] || '#8888a0';
        const actColour = t.action === 'buy' ? 'var(--green)' : 'var(--red)';
        const statusBadge = t.status === 'open'
            ? '<span class="action-tag buy" style="background:rgba(74,124,255,0.15);color:#4a7cff">OPEN</span>'
            : '<span class="action-tag" style="background:rgba(255,255,255,0.06);color:var(--text-muted)">CLOSED</span>';
        const actionBtn = t.status === 'open'
            ? `<button class="clear-btn" onclick="closeTrade(${t.id})">close</button>`
            : `<button class="clear-btn" onclick="deleteLoggedTrade(${t.id})">delete</button>`;
        h += `<tr>
            <td><span class="ticker-link" onclick="loadFromScreener('${t.ticker}')">${t.ticker}</span></td>
            <td><span style="color:${stratColour};font-size:10px;text-transform:uppercase;letter-spacing:0.5px">${stratLabel}</span></td>
            <td style="color:${actColour};font-weight:600;text-transform:uppercase;font-size:10px">${t.action}</td>
            <td>${t.entry_price.toFixed(2)} <span style="color:var(--text-muted);font-size:9px">${t.entry_date}</span></td>
            <td>${t.shares}</td>
            <td>${t.current_price != null ? t.current_price.toFixed(2) : (t.exit_price != null ? t.exit_price.toFixed(2)+'<span style="color:var(--text-muted);font-size:9px"> '+t.exit_date+'</span>' : '-')}</td>
            <td>${t.days_held || (t.exit_date && t.entry_date ? Math.round((new Date(t.exit_date) - new Date(t.entry_date))/86400000) : '-')}</td>
            <td style="color:${plc};font-weight:700">${pl != null ? (pl >= 0 ? '+' : '') + pl + '%' : '-'}</td>
            <td style="color:${plc}">${plv != null ? (plv >= 0 ? '+' : '') + plv.toFixed(2) : '-'}</td>
            <td>${statusBadge}</td>
            <td style="font-size:10px;color:var(--text-muted);max-width:180px">${t.notes || '-'}</td>
            <td>${actionBtn}</td>
        </tr>`;
    });
    h += '</tbody></table>';
...
function filterLog(f, el){
    logFilter = f;
    document.querySelectorAll('#tab-log .chart-range-btn').forEach(b => b.classList.remove('active'));
    el.classList.add('active');
    if (logCache) renderTradeLogList(logCache.trades);
}

async function closeTrade(id){
    if (!confirm('close this trade at current market price?')) return;
    try {
        const r = await fetch('/api/close_trade', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
        const d = await r.json();
        if (d.error){ showStatus('error: '+d.error,'error'); return; }
        showStatus(`closed at ${d.trade.exit_price.toFixed(2)} (${d.trade.realised_pl_pct>=0?'+':''}${d.trade.realised_pl_pct}%)`, 'success');
        refreshTradeLog();
    } catch(e) { showStatus('error: '+e.message,'error'); }
}

async function deleteLoggedTrade(id){
    if (!confirm('delete this trade from log?')) return;
    try {
        await fetch('/api/delete_logged_trade', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
        refreshTradeLog();
    } catch(e) {}
}
...
let paperEquityChart=null, paperProgressPoll=null;

async function loadPaperTraderTab(){
    try{
        const data=await(await fetch('/api/paper/state')).json();
        renderPaperState(data);
    }catch(e){console.error('paper state load failed',e)}
}

function renderPaperState(data){
    const equity=data.equity_gbp||1000, cash=data.cash_gbp||1000;
    const pl=equity-1000, plPct=pl/1000*100;
    document.getElementById('paper-equity').textContent=`£${equity.toFixed(2)}`;
    document.getElementById('paper-cash').textContent=`£${cash.toFixed(2)}`;
    const plEl=document.getElementById('paper-pl');
    plEl.textContent=`£${pl.toFixed(2)} (${pl>=0?'+':''}${plPct.toFixed(2)}%)`;
    plEl.style.color=pl>=0?'#2dd4a0':'#ff6b6b';
    document.getElementById('paper-positions-count').textContent=`${(data.positions||[]).length} / 8`;
    document.getElementById('paper-cycles').textContent=data.cycles_run||0;

    //auto-run toggle button state + status line
    const toggleBtn=document.getElementById('auto-run-toggle-btn');
    const statusDiv=document.getElementById('paper-auto-status');
    const lastCycle=data.last_cycle?new Date(data.last_cycle).toLocaleString('en-GB'):'never';
    if(data.auto_run_enabled){
        toggleBtn.textContent='disable auto-run';
        toggleBtn.classList.add('auto-on');
        statusDiv.innerHTML=`<b style="color:#2dd4a0">auto-run ENABLED</b> - daily at 22:00 local time. Last cycle: ${lastCycle}`;
    }else{
        toggleBtn.textContent='enable auto-run';
        toggleBtn.classList.remove('auto-on');
        statusDiv.innerHTML=`auto-run is disabled. Last cycle: ${lastCycle}`;
    }

    renderPaperPositions(data.positions||[]);
    renderPaperEquityChart(data.equity_curve||[]);
    renderPaperStrategyBreakdown(data.closed_trades||[], data.positions||[]);
    renderPaperActivity(data.activity_log||[]);
}

function renderPaperPositions(positions){
    const c=document.getElementById('paper-positions-list');
    if(!positions.length){c.textContent='no open positions';return}
    let h='<table class="trades-table"><thead><tr><th>ticker</th><th>strategy</th><th>entry</th><th>current</th><th>P/L £</th><th>P/L %</th><th>days</th></tr></thead><tbody>';
    positions.forEach(p=>{
        const days=Math.floor((Date.now()-new Date(p.entry_date).getTime())/86400000);
        const cls=(p.unrealised_pl_gbp||0)>=0?'positive':'negative';
        const label=STRATEGY_LABELS[p.strategy]||p.strategy;
        h+=`<tr><td><b>${p.ticker}</b></td><td>${label}</td><td>${(p.entry_price_native||0).toFixed(2)} ${p.currency}</td><td>${(p.current_price_native||0).toFixed(2)}</td><td class="${cls}">£${(p.unrealised_pl_gbp||0).toFixed(2)}</td><td class="${cls}">${(p.unrealised_pl_pct||0).toFixed(2)}%</td><td>${days}</td></tr>`;
    });
    h+='</tbody></table>';
    c.innerHTML=h;
}
...
function renderPaperEquityChart(curve){
    ...
    paperEquityChart=new Chart(ctx,{
        type:'line',
        data:{labels:curve.map(p=>new Date(p.date).toLocaleDateString('en-GB')),
            datasets:[
                {label:'equity £',data:curve.map(p=>p.equity_gbp),borderColor:'#4a7cff',backgroundColor:'rgba(74,124,255,0.1)',tension:0.2,fill:true},
                {label:'starting £1000',data:curve.map(()=>1000),borderColor:'#666',borderDash:[5,5],pointRadius:0,fill:false}
            ]},
        options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#aaa'}}},scales:{y:{ticks:{color:'#aaa'},grid:{color:'#222'}},x:{ticks:{color:'#aaa'},grid:{color:'#222'}}}}
    });
}

function renderPaperStrategyBreakdown(closed, open){
    const c=document.getElementById('paper-strategy-breakdown');
    const strategies=['cluster','pead','meanrev','momentum','momentum_12_1','week52_high','quality','shareholder_yield','low_beta_trend'];
    let h='<table class="trades-table"><thead><tr><th>strategy</th><th>open</th><th>closed</th><th>wins</th><th>win rate</th><th>avg return</th><th>total P/L £</th></tr></thead><tbody>';
    strategies.forEach(s=>{
        const cs=closed.filter(t=>t.strategy===s);
        const os=open.filter(p=>p.strategy===s);
        const wins=cs.filter(t=>t.realised_pl_gbp>0).length;
        const wr=cs.length?(wins/cs.length*100).toFixed(1)+'%':'n/a';
        const avg=cs.length?(cs.reduce((a,t)=>a+t.realised_pl_pct,0)/cs.length).toFixed(2)+'%':'n/a';
        const tot=cs.reduce((a,t)=>a+t.realised_pl_gbp,0);
        const cls=tot>=0?'positive':'negative';
        const label=STRATEGY_LABELS[s]||s;
        h+=`<tr><td><b>${label}</b></td><td>${os.length}</td><td>${cs.length}</td><td>${wins}</td><td>${wr}</td><td>${avg}</td><td class="${cls}">£${tot.toFixed(2)}</td></tr>`;
    });
    h+='</tbody></table>';
    c.innerHTML=h;
}

function renderPaperActivity(log){
    const c=document.getElementById('paper-activity-log');
    if(!log.length){c.textContent='no activity yet';return}
    const recent=log.slice(-30).reverse();
    let h='<ul class="paper-activity-list">';
    recent.forEach(it=>{
        const dt=new Date(it.date).toLocaleString('en-GB');
        h+=`<li><span class="activity-date">${dt}</span> ${it.event}</li>`;
    });
    h+='</ul>';
    c.innerHTML=h;
}
...
async function pollCycleProgress(){
    try{
        const r=await(await fetch('/api/paper/cycle_progress')).json();
        const fill=document.getElementById('paper-progress-fill');
        const txt=document.getElementById('paper-progress-text');
        const msg=document.getElementById('paper-progress-message');
        if(fill)fill.style.width=(r.progress||0)+'%';
        if(txt)txt.textContent=(r.progress||0)+'%';
        if(msg)msg.textContent=r.message||'';
        if(!r.active){
            clearInterval(paperProgressPoll);paperProgressPoll=null;
            document.getElementById('paper-cycle-progress').classList.add('hidden');
            await loadPaperTraderTab();
            const btn=document.getElementById('run-cycle-btn');
            btn.disabled=false;
            btn.querySelector('.btn-text').textContent='run cycle now';
            btn.querySelector('.btn-loader').classList.add('hidden');
        }
    }catch(e){}
}
...
async function runPaperCycle(){
    const btn=document.getElementById('run-cycle-btn');
    btn.disabled=true;
    btn.querySelector('.btn-text').textContent='running cycle...';
    btn.querySelector('.btn-loader').classList.remove('hidden');
    document.getElementById('paper-cycle-progress').classList.remove('hidden');
    if(paperProgressPoll)clearInterval(paperProgressPoll);
    paperProgressPoll=setInterval(pollCycleProgress, 2000);
    try{
        const res=await fetch('/api/paper/run_cycle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scan_count:550})});
        const data=await res.json();
        if(data.error && data.error !== 'a cycle is already running'){
            console.warn('cycle start error:', data.error);
        }
    }catch(e){
        //if the POST itself fails or times out, the cycle is likely still running in the background thread
        //polling picks up the real state so we just log and let it continue
        console.log('cycle start request error (cycle may still be running):', e.message);
    }
    //pollCycleProgress handles button cleanup when active flips to false
}

async function toggleAutoRun(){
    try{
        const r=await(await fetch('/api/paper/toggle_auto',{method:'POST'})).json();
        await loadPaperTraderTab();
        if(r.enabled){
            alert('auto-run ENABLED. Cycles will fire daily at 22:00 local time as long as merlin is running.');
        }
    }catch(e){alert('toggle failed: '+e.message)}
}

async function resetPaperPortfolio(){
    if(!confirm('reset paper portfolio to £1000 and wipe all paper trade history?'))return;
