/*
merlin 5.5 - ensemble + screener + insider + fundamentals + ai assessment
*/
...
function switchTab(tab){document.querySelectorAll('.tab-btn').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.tab-content').forEach(c=>c.classList.add('hidden'));document.getElementById('tab-'+tab).classList.remove('hidden');event.target.classList.add('active')}
...
function renderFundamentals(f){
    const sec=document.getElementById('fundamentals-section');
    if(!f||f.error){sec.classList.add('hidden');return}
    sec.classList.remove('hidden');
    const a=f.assessment||{};

    //verdict badge
    const vb=document.getElementById('fund-verdict');
    vb.className='fund-verdict-badge '+(a.verdict||'unknown');
    const verdictLabels={undervalued:'potentially undervalued',overvalued:'potentially overvalued',fair_value:'fairly valued',unknown:'unknown'};
    vb.textContent=verdictLabels[a.verdict]||'unknown';

    //assessment text
    const strCol=a.strength==='strong'?'var(--green)':a.strength==='moderate'?'var(--amber)':'var(--red)';
    document.getElementById('fund-assessment').innerHTML=`
        <div class="fund-assessment-summary">${a.summary||''}</div>
        <div class="fund-strength"><span class="label">fundamental strength: </span><span style="color:${strCol};font-weight:600">${a.strength||'unknown'}</span> — ${a.strength_text||''}</div>
    `;

    //metric cards
    const betaCol=f.beta?(f.beta>1.5?'var(--red)':f.beta>1.1?'var(--amber)':'var(--green)'):'var(--text-secondary)';
    const peCol=f.pe_ratio?(f.pe_ratio<0?'var(--red)':f.pe_ratio<20?'var(--green)':f.pe_ratio<40?'var(--amber)':'var(--red)'):'var(--text-secondary)';
    ...
    document.getElementById('fund-grid').innerHTML=`
        ...
        <div class="fund-card"><span class="fund-card-label">P/E ratio</span><span class="fund-card-value" style="color:${peCol}">${f.pe_ratio||'n/a'}</span><div class="fund-card-sub">${f.forward_pe?'fwd: '+f.forward_pe:''}</div></div>
        <div class="fund-card"><span class="fund-card-label">beta</span><span class="fund-card-value" style="color:${betaCol}">${f.beta||'n/a'}</span><div class="fund-card-sub">volatility vs market</div></div>
        <div class="fund-card"><span class="fund-card-label">dividend yield</span><span class="fund-card-value">${f.dividend_yield?f.dividend_yield+'%':'0%'}</span><div class="fund-card-sub">annual</div></div>
        <div class="fund-card"><span class="fund-card-label">avg volume</span><span class="fund-card-value">${f.avg_volume_str||'n/a'}</span><div class="fund-card-sub">daily</div></div>
        ...
        <div class="fund-card"><span class="fund-card-label">debt/equity</span><span class="fund-card-value" style="color:${f.debt_to_equity&&f.debt_to_equity<80?'var(--green)':f.debt_to_equity&&f.debt_to_equity<150?'var(--amber)':'var(--red)'}">${f.debt_to_equity||'n/a'}</span></div>
    `;

    //assessment points
    const pts=a.points||[];
    document.getElementById('fund-points').innerHTML=pts.map(p=>`<div class="fund-point"><div class="fund-point-dot ${p.type}"></div><span>${p.text}</span></div>`).join('');
}

function renderInsider(ins){
    const sec=document.getElementById('insider-section');
    if(!ins.transactions||!ins.transactions.length){sec.classList.add('hidden');return}
    sec.classList.remove('hidden');
    const badge=document.getElementById('insider-badge');
    const s=ins.sentiment;
    badge.className='news-badge '+(s==='bullish'?'positive':s==='bearish'?'negative':'neutral');
    badge.textContent=s;
    document.getElementById('insider-summary').innerHTML=`
        <div class="stat-item"><span class="stat-label">exec buys</span><span class="stat-value" style="color:var(--green)">${ins.exec_buys}</span></div>
        <div class="stat-item"><span class="stat-label">exec sells</span><span class="stat-value" style="color:var(--red)">${ins.exec_sells}</span></div>
        <div class="stat-item"><span class="stat-label">total</span><span class="stat-value">${ins.all_buys+ins.all_sells}</span></div>`;
    document.getElementById('insider-list').innerHTML=ins.transactions.filter(t=>t.action!=='other').slice(0,15).map(t=>{
        const val=t.value?'$'+(t.value>=1e6?(t.value/1e6).toFixed(1)+'M':t.value>=1e3?(t.value/1e3).toFixed(0)+'K':t.value.toFixed(0)):'';
        return `<div class="insider-item"><span class="insider-name">${t.name}</span><span class="insider-title">${t.title}</span><span class="insider-action ${t.action}">${t.action}</span><span class="insider-details">${t.shares?t.shares.toLocaleString()+' shares':''}${val?' · '+val:''}</span></div>`}).join('');
}
...
async function trainModel(){
    ...
    try{
        const r=await fetch('/api/train',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ticker:currentTicker,epochs:ep})});
        ...
        const poll=setInterval(async()=>{try{const s=await(await fetch('/api/training_status')).json();
            ...
            if(s.error){clearInterval(poll);showStatus('error: '+s.error,'error');rb(btn)}}catch(e){}},1000);
    }catch(e){showStatus('error: '+e.message,'error');rb(btn)}
}
...
function renderBT(bt){
    ...
    const d=bt.ensemble_direction_accuracy,badge=document.getElementById('backtest-confidence');
    let c=d>=65?'high':d>=55?'medium':'low';badge.className='confidence-badge confidence-'+c;badge.textContent=c+' confidence';
...
async function runPrediction(tf,el){
    if(!currentTicker)return;document.querySelectorAll('.timeframe-btn').forEach(b=>{b.classList.remove('active');b.classList.add('loading')});el.classList.add('active');showStatus(`predicting ${tf}...`);
    ...
    document.getElementById('prediction-results').classList.remove('hidden');renderVP(data);renderPC(data);renderA(data);showStatus(tf+' prediction complete','success')}
...
<div class="stat-item"><span class="stat-label">insider</span><span class="stat-value" style="color:${ic}">${a.insider_sentiment||'n/a'} (${a.exec_buys||0}B/${a.exec_sells||0}S)</span></div>
</div>${sb}`}
...
if(s.error){clearInterval(poll);btn.querySelector('.btn-text').textContent='run screener';btn.querySelector('.btn-loader').classList.add('hidden');btn.disabled=false}}catch(e){}},1500)}catch(e){btn.querySelector('.btn-text').textContent='run screener';btn.querySelector('.btn-loader').classList.add('hidden');btn.disabled=false}}
...
let h=`<table class="screener-table"><thead><tr><th>#</th><th>ticker</th><th>price</th><th>1d</th><th>5d</th><th>RSI</th><th>P/E</th><th>beta</th><th>mkt cap</th><th>score</th><th>valuation</th><th>strength</th><th>signals</th></tr></thead><tbody>`;
results.forEach((r,i)=>{
const dc=r.direction==='bullish'?'var(--green)':r.direction==='bearish'?'var(--red)':'var(--amber)';
const vc=r.valuation==='undervalued'?'var(--green)':r.valuation==='overvalued'?'var(--red)':'var(--amber)';
const sc=r.strength==='strong'?'var(--green)':r.strength==='weak'?'var(--red)':'var(--amber)';
const sigs=(r.signals||[]).map(s=>`<span class="signal-tag">${s}</span>`).join('');
...
function loadFromScreener(ticker){document.getElementById('ticker-input').value=ticker;switchTab('analyse');document.querySelectorAll('.tab-btn').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.tab-btn')[0].classList.add('active');fetchData()}
