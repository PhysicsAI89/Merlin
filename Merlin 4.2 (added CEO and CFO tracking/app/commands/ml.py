...
def estimate_sell_window(symbol: str, buy_threshold: float = 0.55, hold_grid: T.Iterable[int] = (3, 5, 10)) -> dict:
"""Train Merlin's classifier and choose the hold length that worked best recently.
Approach (fast):
• Train on history up to last 90 days; predict probabilities on last 90 days
• Whenever Pr(up)≥buy_threshold on day t, compute forward return over H days
• Pick H with best average forward return; return stats
"""
prices = load_prices(symbol)
if prices.empty:
...
if df.shape[0] < 300:
return {"symbol": symbol, "error": "Not enough history (<300 rows)"}
...
proba = pipe.predict_proba(test[FEATURE_COLUMNS].values)[:, 1]
test = test.copy()
test["proba_up"] = proba
...
results = []
for H in hold_grid:
fwd = test["close_price"].pct_change(periods=H).shift(-H)
entries = test[test["proba_up"] >= buy_threshold].index
avg = float(fwd.loc[entries].mean()) if len(entries) else float("nan")
med = float(fwd.loc[entries].median()) if len(entries) else float("nan")
n_trades = int(len(entries))
results.append({"H": int(H), "avg_return": avg, "median_return": med, "n_trades": n_trades})


dfres = pd.DataFrame(results).dropna()
if dfres.empty:
return {"symbol": symbol, "error": "No qualifying entries in backtest"}
best = dfres.sort_values("avg_return", ascending=False).iloc[0]
return {
"symbol": symbol,
"best_hold": int(best["H"]),
"avg_return": float(best["avg_return"]),
"median_return": float(best["median_return"]),
